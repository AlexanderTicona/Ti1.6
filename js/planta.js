...
function dibujarPlanta() {
    ...
    // --- NUEVA LÓGICA DE GRILLA MANUAL ---
    const gridInput = document.getElementById('inpGridPlanta');
    // Leemos el valor del input, si no existe o es error, usamos 100 por defecto
    let gSize = gridInput ? parseFloat(gridInput.value) : 200;
    
    // Seguridad para evitar bucles infinitos si el usuario pone 0 o negativo
    if (isNaN(gSize) || gSize <= 0) gSize = 100;
