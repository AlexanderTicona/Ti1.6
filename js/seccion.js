...
function dibujarSeccion(seccion) {
    ...
    // 9. MIRA DE SELECCIÓN (Corregida para Coordenadas Directas)
    if (appState.lastClick) {
        // IMPORTANTE: Ya no usamos toX() ni toY() porque appState.lastClick 
        // ya viene en coordenadas de dibujo desde updateHUD.
        const px = appState.lastClick.x;
        const py = appState.lastClick.y;

        ctx.save();
        // Dibujo del brillo/bruma del punto
